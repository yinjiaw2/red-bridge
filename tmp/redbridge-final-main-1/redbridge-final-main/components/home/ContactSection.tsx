...
import { contactCards } from "./home-data";
      ...
      <div className="home-inner">
        <div className="mx-auto max-w-[820px] text-center">
          ...
          <h2 className="mt-3 font-display text-[2.7rem] leading-[0.98] text-[var(--text-main)] md:text-[3.2rem]">
            Your First Step is <span className="italic text-[var(--accent)]">Free</span>
          </h2>
          <p className="mx-auto mt-4 max-w-[760px] text-lg leading-8 text-[var(--text-sub)]">
            Book a no-obligation consultation. We&apos;ll give you an honest assessment — including telling you if none of our programs are the right fit for your situation.
          </p>
        </div>

        <div className="mt-9 rounded-[24px] border border-[var(--border-soft)] bg-[var(--bg-card)] px-6 py-7 shadow-[var(--shadow)] md:px-8 md:py-8">
            ...
            {contactCards.map((card) => {
              ...
              return (
                <Link
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-[20px] border border-[var(--border-soft)] bg-[var(--bg)] px-5 py-6 text-center"
                >
                  ...
                  <div className="mt-4 space-y-1 text-base text-[var(--text-sub)]">
                    {card.body.map((line) => (
                      <p key={line} className={card.title === "Call Us" || card.title === "Email Us" ? "font-semibold text-[var(--text-main)]" : ""}>
                        {line}
                      </p>
                    ))}
                  </div>
