...
export default function PathwaysSection() {
  ...
  const handlePrimaryAction = () => {
    if (activePanel?.buttonHref.startsWith("/contact")) {
      router.push(activePanel.buttonHref);
      return;
    }
        ...
        {activePanel ? (
          <div className="overflow-hidden rounded-[12px] border border-border bg-card shadow-[0_4px_24px_rgba(42,31,20,0.08)]">
            <div className="bg-primary px-8 py-10 text-white">
              ...
              <p className="mt-3 max-w-[680px] text-[15px] leading-7 text-white/75">
            ...
            <div className="p-8 md:p-10">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {activePanel.chips.map((chip) => (
                  <span
                    key={chip}
                    className="rounded-full border border-primary/20 bg-primary/5 px-4 py-2 text-[12px] font-medium text-primary"
                  >
                    {chip}
                  </span>
                ))}
