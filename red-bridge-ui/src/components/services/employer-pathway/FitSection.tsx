          ...
          <article className="rounded-[10px] border border-border bg-card p-8">
            ...
            <div className="space-y-3">
              {yesItems.map((item) => (
                <p
                  key={item}
                  className="flex items-start gap-3 text-[14px] leading-7 text-muted-foreground"
                >
                  <Check
                    size={14}
                    className="mt-[4px] shrink-0 text-[#2d6a4f]"
                  />
                  {item}
                </p>
              ))}
              ...
              <p className="mt-5 text-[13px] leading-6 text-muted-foreground">
                ...
                {no.noteLinkText && no.noteLinkHref ? (
                  <Link
                    ...
                    className="ml-4 inline-flex rounded-full border border-primary/20 bg-primary/10 px-4 py-2 text-[12px] font-semibold text-primary transition-all hover:-translate-y-0.5 hover:bg-primary hover:text-white hover:shadow-md"
                  >
                    {no.noteLinkText}
                  </Link>
                ) : null}
