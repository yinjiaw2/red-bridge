...
const SECTORS = [
  { label: 'Technology & ICT', count: 12, iconPath: 'M9 3H5a2 2 0 00-2 2v4m6-6h10a2 2 0 012 2v4M9 3v18m0 0h10a2 2 0 002-2V9M9 21H5a2 2 0 01-2-2V9m0 0h18', highlight: ['Software engineers & developers', 'ICT business & systems analysts', 'Database administrators', 'ICT project managers'], assessingBody: 'ACS' },
  { label: 'Accounting & Finance', count: 8, iconPath: 'M9 7H6a2 2 0 00-2 2v9a2 2 0 002 2h9a2 2 0 002-2v-3M9 7V5a2 2 0 012-2h2M9 7h6m0 0l2-2m-2 2l2 2', highlight: ['Management accountants', 'Financial analysts', 'Tax accountants', 'External auditors'], assessingBody: 'CAANZ or CPA' },
  { label: 'Marketing & Communications', count: 6, iconPath: 'M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z', highlight: ['Marketing specialists', 'Digital marketing managers', 'Market research analysts', 'Business development managers'], assessingBody: 'AMI' },
  { label: 'Engineering', count: 5, iconPath: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z', highlight: ['Civil & structural engineers', 'Mechanical engineers', 'Electrical engineers', 'Project engineers'], assessingBody: 'Engineers Australia' },
];

const VERIFY_STEPS = [
  { icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', label: 'DOHA Sponsorship Check', detail: 'Active SBS confirmed against department records before any introduction.' },
  { icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4', label: 'Business Legitimacy Review', detail: 'ABN verification, trading history, and genuine vacancy review for each role.' },
  { icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z', label: 'TSMIT Salary Compliance', detail: 'Confirmed against current Temporary Skilled Migration Income Threshold before match.' },
  { icon: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15', label: 'Ongoing Recheck', detail: 'Status confirmed again at nomination stage — sponsorship can be revoked between checks.' },
];
    ...
    <section style={{ background: B.cream, borderTop: `1px solid ${B.divider}`, padding: '88px 24px', fontFamily: 'var(--font-dm-sans, "DM Sans", system-ui, sans-serif)' }}>
      <div style={{ maxWidth: 1060, margin: '0 auto' }}>
        <div ref={headerRef} style={{ display: 'flex', flexWrap: 'wrap', gap: 24, justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 44 }}>
          <div>
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: B.red, display: 'block', marginBottom: 12 }}>Employer Network</span>
            <h2 style={{ fontSize: 'clamp(24px, 3.5vw, 36px)', fontWeight: 700, color: B.textDeep, letterSpacing: '-0.02em', margin: '0 0 8px' }}>
              We Match by{' '}<em style={{ fontStyle: 'italic', color: B.red }}>Industry, Not Category</em>
            </h2>
            <p style={{ fontSize: 14.5, color: B.textMuted, margin: 0, maxWidth: 440, lineHeight: 1.6 }}>Employers in our network operate in four sectors where 482 sponsorship is both common and viable.</p>
          </div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: B.redLight, border: `1px solid ${B.redMid}`, borderRadius: 8, padding: '10px 16px', flexShrink: 0 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: B.red }}>All employers individually verified</span>
          </div>
        </div>
            ...
            <div key={sector.label} ref={(el) => { cardRefs.current[i] = el; }} style={{ background: B.bg, border: `1px solid ${B.divider}`, borderRadius: 14, padding: '22px 20px', display: 'flex', flexDirection: 'column', gap: 14, opacity: 0, transform: 'translateY(22px)', transition: `opacity 0.5s ease ${i * 0.07}s, transform 0.5s ease ${i * 0.07}s` }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <span style={{ fontSize: 10, color: B.textMuted, display: 'block', lineHeight: 1.3 }}>employers</span>
                </div>
              </div>
              <div>
              ...
              <div style={{ marginTop: 'auto' }}>
                <span style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.05em', color: B.navy, background: '#eef1f8', border: '1px solid #c8d3e8', borderRadius: 5, padding: '3px 8px', display: 'inline-block' }}>
                  Skills assessment: {sector.assessingBody}
                </span>
              </div>
            </div>
          ))}
        ...
        <div ref={verifyRef} style={{ background: B.navy, borderRadius: 16, padding: '32px 32px 28px' }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: B.amber, margin: '0 0 20px' }}>Our 4-Step Employer Verification Process</p>
        ...
        <div style={{ textAlign: 'center', marginTop: 32 }}>
          <Link href={`/contact?ctasrc=employer_v2_industry_network_cta&locale=${locale}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 24px', background: B.red, color: '#fff', fontWeight: 700, fontSize: 14, borderRadius: 9, textDecoration: 'none' }}>
            Book a free assessment — see who&apos;s hiring for your occupation
