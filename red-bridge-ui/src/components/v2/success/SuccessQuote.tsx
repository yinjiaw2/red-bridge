...
const B = {
  ...
  cream: '#fbf4ec', amber: '#efb64f', textDeep: '#22150f', textMuted: '#7a6a60',
  divider: '#ece5de', badgeBg: '#ead6bf',
};

const PRIMARY_QUOTE = { quote: "Two agencies had taken my money and given me nothing I could use. RedBridge was the first to tell me exactly what I didn't qualify for — before I paid a cent. That honesty is why I trust them with my PR.", name: 'W.L.', role: 'Accountant', city: 'Melbourne', visaType: '482 → 186', initials: 'WL' };
const SECONDARY_QUOTE = { quote: 'The ACS assessment process felt impossible until RedBridge structured my project portfolio correctly. First attempt, no rejections.', name: 'L.T.', role: 'Software Engineer', city: 'Sydney', visaType: '189 — ACS', initials: 'LT' };
...
function QuoteCard({ q, isFeatured, delay }: { q: typeof PRIMARY_QUOTE; isFeatured: boolean; delay: number }) {
  ...
  return (
    <div ref={ref} style={{ background: isFeatured ? B.navy : B.cream, border: `1px solid ${isFeatured ? 'transparent' : B.divider}`, borderRadius: 18, padding: '32px 32px 28px', display: 'flex', flexDirection: 'column', gap: 20, position: 'relative', overflow: 'hidden' }}>
      ...
      <blockquote style={{ fontSize: isFeatured ? 18 : 15, fontStyle: 'italic', color: isFeatured ? 'rgba(255,255,255,0.9)' : '#1f1f1f', lineHeight: 1.7, margin: 0, position: 'relative', zIndex: 1, letterSpacing: '-0.01em' }}>
        &ldquo;{q.quote}&rdquo;
      </blockquote>
    ...
    <section style={{ background: B.bg, padding: '72px 24px 64px', fontFamily: 'var(--font-dm-sans, "DM Sans", system-ui, sans-serif)' }}>
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        <div ref={headerRef} style={{ textAlign: 'center', marginBottom: 36 }}>
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: B.red }}>In Their Own Words</span>
          <p style={{ fontSize: 15, color: B.textMuted, margin: '10px auto 0', lineHeight: 1.6, maxWidth: 440 }}>Real clients. Real feedback. Unscripted.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 18 }}>
          <QuoteCard q={PRIMARY_QUOTE} isFeatured delay={0.05} />
          <QuoteCard q={SECONDARY_QUOTE} isFeatured={false} delay={0.15} />
        </div>
        <p style={{ textAlign: 'center', fontSize: 11.5, color: B.textMuted, marginTop: 20, lineHeight: 1.5 }}>
          Clients quoted with initials, occupation, and city only. All testimonials are genuine — collected as written feedback from verified RedBridge clients.
        </p>
      </div>
    </section>
  );
