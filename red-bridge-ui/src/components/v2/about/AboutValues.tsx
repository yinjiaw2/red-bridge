...
const VALUE_STYLE = [
  { accent: B.red,      bg: B.redLight,   border: B.redMid },
  { accent: B.navy,     bg: '#eef1f8',    border: '#c8d3e8' },
  { accent: '#2d6a4f',  bg: '#eaf4ee',    border: '#b8d4b8' },
  { accent: '#7a5000',  bg: '#fdf7ee',    border: '#f0d9a0' },
  { accent: B.red,      bg: B.redLight,   border: B.redMid },
  { accent: B.navy,     bg: '#eef1f8',    border: '#c8d3e8' },
];
...
export default function AboutValues() {
  const t = useTranslations('v2.about.values');
  const locale = useLocale();
  const VALUES_DATA = (t.raw('values') as { label: string; body: string }[]).map((v, i) => ({
    ...v,
    iconPath: VALUE_ICON_PATHS[i],
    ...VALUE_STYLE[i],
  }));
